...
Do the load peaks and nadirs the same over time? 
```{r}
ma_sector_hour %>% 
  summarise(max = max(LoadMW),
            min = min(LoadMW), 
...
Sometimes you just need to stop and think about what you are actually trying to show. 
```{r}
ma_sector_hour %>% 
  summarise(max = max(LoadMW),
            min = min(LoadMW), 
...
ma_sector_hour %>% 
  ...
  geom_point(aes(max, max_hour, color = factor(Year)), shape = 21) + #outline
  facet_wrap(~month)
```
